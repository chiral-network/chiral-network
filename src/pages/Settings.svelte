  ...
  <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">Control what starts automatically after login</p>
      </div>
    </div>

    <button
      onclick={toggleAutoStartMining}
      class="w-full flex items-center justify-between gap-4 py-3 px-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
      role="switch"
      aria-checked={$settings.autoStartMining}
      title="Automatically start mining when you log in"
    >
      <div class="text-left">
        <p class="text-sm font-medium text-gray-900 dark:text-white">Auto-start mining on login</p>
        <p class="text-sm text-gray-500 dark:text-gray-400">
          Starts mining automatically after login when your node is available. Default: off.
        </p>
      </div>
      <div class="relative w-11 h-6 rounded-full shrink-0 transition-colors {$settings.autoStartMining ? 'bg-primary-500' : 'bg-gray-300 dark:bg-gray-600'}">
        <span
          class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform {$settings.autoStartMining ? 'translate-x-5' : 'translate-x-0'}"
        ></span>
      </div>
    </button>
  </div>
