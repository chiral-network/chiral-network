<script lang="ts">
 ...
 case'file_payment':
 return { bgColor:'bg-purple-900/30', iconColor:'text-purple-400' };
 case'file_sale':
 return { bgColor:'bg-emerald-900/30', iconColor:'text-emerald-400' };
 case'receive':
 return { bgColor:'bg-green-500/[0.08]', iconColor:'text-green-400' };
 case'send':
 return { bgColor:'bg-red-500/[0.08]', iconColor:'text-red-400' };
 default:
 return isIncoming(tx)
 ? { bgColor:'bg-green-500/[0.08]', iconColor:'text-green-400' }
 : { bgColor:'bg-red-500/[0.08]', iconColor:'text-red-400' };
 }
 }
...
<div class="p-4 sm:p-6 space-y-6 max-w-6xl mx-auto">
 ...
 <div class="p-3 bg-black/15 rounded-full">
 ...
 <span class="px-3 py-1 bg-black/15 rounded-full text-sm">
 {$networkConnected ?'Connected' :'Disconnected'}
 </span>
 </div>

 <!-- Balance Display -->
 <div class="bg-black/10 rounded-xl p-4 mt-4">
 <div class="flex items-center justify-between">
 <div>
 ...
 {:else if balance ==='--'}
 <span class="text-xl font-light text-white/50">--</span>
 <span class="text-white/50 text-sm">Connecting to network...</span>
 {:else}
 ...
 class="p-2 hover:bg-black/10 rounded-lg transition-colors disabled:opacity-50"
 ...
 class="flex-1 px-4 py-3 bg-white/[0.07] border border-gray-800/60 rounded-lg font-mono text-sm"
 />
 <button
 onclick={() => copyToClipboard($walletAccount!.address,'address')}
 class="p-3 hover:bg-white/[0.08] rounded-lg transition-colors border border-gray-800/60 focus:outline-none focus:border-blue-400/40"
 title="Copy address"
 >
 {#if copied ==='address'}
 ...
 class="w-full px-4 py-3 bg-white/[0.07] border border-gray-800/60 rounded-lg font-mono text-sm pr-12"
 />
 <button
 onclick={() => privateKeyVisible = !privateKeyVisible}
 class="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-white/[0.1] rounded transition-colors"
 ...
 <p class="text-sm text-white/50">Transfer CHI to another address</p>
 </div>
 </div>
 ...
 class="flex-1 px-3 py-2 border border-gray-800/60 rounded-lg text-sm focus:outline-none focus:border-blue-400/40"
 ...
 class="p-2 text-white/50 hover:text-white/70"
 >
 <X class="w-4 h-4" />
 </button>
 </div>
 ...
 placeholder="0x..."
 class="w-full px-4 py-3 border border-gray-800/60 rounded-lg font-mono text-sm focus:outline-none focus:border-blue-400/40"
 />
 </div>
 ...
 placeholder="0.00"
 class="w-full px-4 py-3 border border-gray-800/60 rounded-lg text-sm focus:outline-none focus:border-blue-400/40"
 />
 <button
 ...
 <div class="group flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/[0.05] transition-colors cursor-pointer {recipientAddress.toLowerCase() === r.address.toLowerCase() ?'bg-blue-500/[0.06] border-blue-400' :''}"
 role="button"
 tabindex="0"
 ...
 <p class="text-xs font-mono text-white/40 truncate">{r.address}</p>
 </div>
 <button
 ...
 <div class="bg-white/[0.07] rounded-lg p-4 space-y-3">
 <div class="flex justify-between">
 <span class="text-sm text-white/50">From</span>
 <span class="text-sm font-mono">{formatAddress($walletAccount?.address ||'')}</span>
 </div>
 <div class="flex justify-between">
 <span class="text-sm text-white/50">To</span>
 <span class="text-sm">
 {#if getRecipientLabel(recipientAddress)}
 <span class="font-medium">{getRecipientLabel(recipientAddress)}</span>
 <span class="font-mono text-white/40 ml-1">({formatAddress(recipientAddress)})</span>
 {:else}
 ...
 <span class="text-sm text-white/50">Amount</span>
 <span class="text-lg font-bold text-blue-400">{sendAmount} CHI</span>
 </div>
 </div>

 <div class="bg-yellow-900/30 border border-yellow-800 rounded-lg p-3">
 <p class="text-sm text-yellow-800">
 ...
 disabled={isSending}
 class="flex-1 px-4 py-2 border border-gray-800/60 rounded-lg hover:bg-white/[0.05] transition-colors disabled:opacity-50"
 >
 Back
 </button>
 <button
 ...
 <p class="text-sm text-white/50">Recent transactions</p>
 </div>
 </div>
 <button
 onclick={loadTransactionHistory}
 disabled={isLoadingHistory}
 class="p-2 hover:bg-white/[0.08] rounded-lg transition-colors disabled:opacity-50"
 ...
 <div class="bg-white/[0.07] rounded-lg hover:bg-white/[0.1] transition-colors">
 <!-- Main row -->
 <button
 ...
 <span class="text-xs px-2 py-0.5 bg-white/[0.04] rounded-full">{tx.status}</span>
 </div>
 ...
 <!-- Expanded details -->
 {#if isExpanded}
 <div class="px-4 pb-4 pt-1 border-t border-gray-800/60 space-y-2">
 ...
 class="p-1 hover:bg-white/[0.1] rounded transition-colors flex-shrink-0"
 ...
 <p class="text-sm text-white/50">Elo score from recent transfers, earnings, and user ratings</p>
 </div>
 </div>
 <AccountReputation />
 </div>
 ...
 <p class="text-white/50 mb-6">Please create or import a wallet to view account details.</p>
 </div>
 {/if}
</div>
...
{#if showLogoutModal}
 ...
 <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" role="dialog" aria-modal="true" tabindex="-1" onclick={() => showLogoutModal = false} onkeydown={(e) => e.key ==='Escape' && (showLogoutModal = false)}>
 <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
 ...
 class="flex-1 px-4 py-2 border border-gray-800/60 rounded-lg hover:bg-white/[0.05] transition-colors"
 >
 Cancel
 </button>
 <button
