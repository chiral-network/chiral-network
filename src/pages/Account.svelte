<script lang="ts">
  ...
  function deobfuscate(base64Text: string): string {
    try {
      const resultBytes = [...atob(base64Text)].map((char) =>
        char.charCodeAt(0),
      );
      const keyBytes = new TextEncoder().encode(OBFUSCATION_KEY);
      const textBytes = resultBytes.map(
        (byte, i) => byte ^ keyBytes[i % keyBytes.length],
      );
  ...
  let Html5QrcodeScanner: InstanceType<typeof Html5QrcodeScannerClass> | null =
    null;
  ...
  $: filteredTransactions = $transactions
    .filter((tx) => {
      ...
      const fromOk =
        !filterDateFrom || txDate >= new Date(filterDateFrom + "T00:00:00"); // the start of day
      const toOk =
        !filterDateTo || txDate <= new Date(filterDateTo + "T23:59:59"); // and the end of day to include full date ranges
  ...
  $: {
    if (rawAmountInput === "") {
      ...
      if (isNaN(inputValue) || inputValue <= 0) {
        ...
        validationWarning = tr("errors.amount.insufficient", {
          values: { more: (inputValue - $wallet.balance).toFixed(2) },
        });
        isAmountValid = false;
        sendAmount = 0;
        ...
        validationWarning = tr("errors.amount.insufficientWithFee", {
          values: {
            more: (inputValue + estimatedFeeNumeric - $wallet.balance).toFixed(
              2,
            ),
          },
        });
  ...
  $: estimatedFeeNumeric =
    rawAmountInput && parseFloat(rawAmountInput) > 0
      ? parseFloat(
          (
            parseFloat(rawAmountInput) *
            { low: 0.0025, market: 0.005, fast: 0.01 }[feePreset]
          ).toFixed(4),
        )
      : 0;
  $: estimatedFeeDisplay =
    rawAmountInput && parseFloat(rawAmountInput) > 0
      ? `${estimatedFeeNumeric.toFixed(4)} Chiral`
      : "—";
  ...
  $: keystoreOptions = keystoreAccounts.map((acc) => ({
    value: acc,
    label: acc,
  }));
  ...
  function isAddressBlacklisted(address: string): boolean {
    return $blacklist.some(
      (entry) => entry.chiral_address.toLowerCase() === address.toLowerCase(),
    );
  }
  ...
  function sendTransaction() {
    if (!isAddressValid || !isAmountValid || !isAddressValid || sendAmount <= 0)
      return;
  ...
  async function createChiralAccount() {
    ...
    try {
      ...
      if (isTauri) {
        account = (await invoke("create_chiral_account")) as {
          address: string;
          private_key: string;
          blacklist: Object[];
        };
  ...
  async function saveToKeystore() {
    ...
    try {
      if (isTauri) {
        ...
        // Simulate for web
        console.log(
          "Simulating save to keystore with password:",
          keystorePassword,
        );
  ...
  async function importChiralAccount() {
    ...
    try {
      ...
      if (isTauri) {
        account = (await invoke("import_chiral_account", {
      ...
      fileInput.onchange = async (event) => {
        ...
        try {
          ...
          if (!accountData.privateKey) {
            showToast(
              "Invalid file format: privateKey field not found",
              "error",
            );
            return;
          }
  ...
  async function completeMnemonicWizard(ev: {
    mnemonic: string;
    passphrase: string;
    account: {
      address: string;
    ...
    hdAccounts = [
      {
        index: ev.account.index,
        change: ev.account.change,
        address: ev.account.address,
        privateKeyHex: ev.account.privateKeyHex,
        label: ev.name || "Account 0",
      },
    ];
    // set as active
    etcAccount.set({
      address: ev.account.address,
      private_key: "0x" + ev.account.privateKeyHex,
    });
  ...
  function loadSavedPassword(address: string) {
    try {
      const savedPasswordsRaw = localStorage.getItem(
  ...
  async function loadFromKeystore() {
    ...
    try {
      if (isTauri) {
        ...
        if (
          decryptedAccount.address.toLowerCase() !==
          selectedKeystoreAccount.toLowerCase()
        ) {
  ...
  function saveOrClearPassword(address: string, password: string) {
    try {
      ...
      let savedPasswords = savedPasswordsRaw
        ? JSON.parse(savedPasswordsRaw)
        : {};
  ...
  async function setup2FA() {
    ...
    try {
      ...
      const account = encodeURIComponent(
        $etcAccount?.address.slice(0, 16) || "user",
      );
  ...
  $: isBlacklistFormValid =
    newBlacklistEntry.reason.trim() !== "" && isBlacklistAddressValid;

  // Filtered blacklist for search
  $: filteredBlacklist = $blacklist.filter(
    (entry) =>
      entry.chiral_address
        .toLowerCase()
        .includes(blacklistSearch.toLowerCase()) ||
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      try {
        ...
        if (data.blacklist && Array.isArray(data.blacklist)) {
          const imported = data.blacklist
            .filter(
              (entry: Partial<BlacklistEntry>) =>
                entry.chiral_address &&
                entry.reason &&
...
<div class="space-y-6">
          ...
          <div class="space-y-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <Button
                variant="outline"
                class="w-full"
                on:click={openCreateMnemonic}
              >
                <KeyRound class="h-4 w-4 mr-2" />
                {$t("wallet.hd.create_via_phrase")}
              </Button>
              <Button
                variant="outline"
                class="w-full"
                on:click={openImportMnemonic}
              >
                <Import class="h-4 w-4 mr-2" />
                {$t("wallet.hd.import_phrase")}
              </Button>
            </div>
            ...
            <div class="space-y-3">
              ...
              {#if keystoreAccounts.length > 0}
                <div class="space-y-2">
                  <div>
                    <Label for="keystore-account"
                      >{$t("keystore.load.select")}</Label
                    >
                    <div class="mt-1">
                      <DropDown
                        id="keystore-account"
                  ...
                  <div>
                    <Label for="keystore-password"
                      >{$t("placeholders.password")}</Label
                    >
                    <Input
                  ...
                  <div class="flex items-center space-x-2">
                    <input
                      ...
                      bind:checked={rememberKeystorePassword}
                    />
                    <label
                  ...
                  <div
                    class="text-xs text-muted-foreground p-2 bg-yellow-50 border border-yellow-200 rounded-md"
                  >
                  ...
                  <Button
                    ...
                    disabled={!selectedKeystoreAccount ||
                      !loadKeystorePassword ||
                  ...
                  {#if keystoreLoadMessage}
                    <p
                      class="text-xs text-center {keystoreLoadMessage
                        .toLowerCase()
                        .includes('success')
                        ? 'text-green-600'
                        : 'text-red-600'}"
        ...
        {:else}
          <div>
            ...
            <p class="text-2xl font-bold">
              {$wallet.balance.toFixed(2)} Chiral
            </p>
          </div>
          ...
          <div class="mt-6">
            ...
            <div class="flex items-center gap-2 mt-1">
              <p class="font-mono text-sm">
                {$etcAccount.address.slice(0, 10)}...{$etcAccount.address.slice(
                  -8,
                )}
              </p>
              <div class="relative">
                ...
                {#if copyMessage}
                  <span
                    class="absolute top-full left-1/2 transform -translate-x-1/2 text-xs text-green-600 mt-1 whitespace-nowrap"
                    >{copyMessage}</span
                  >
                {/if}
              </div>
              <Button
                size="sm"
                variant="outline"
                on:click={generateAndShowQrCode}
                title={$t("tooltips.showQr")}
                aria-label={$t("aria.showQr")}
              >
                <svg
                  ...
                  ><path
                    d="M5 5h3v3H5zM5 16h3v3H5zM16 5h3v3h-3zM16 16h3v3h-3zM10.5 5h3M10.5 19h3M5 10.5v3M19 10.5v3M10.5 10.5h3v3h-3z"
                  /></svg
                >
              </Button>
              {#if showQrCodeModal}
                ...
                >
                  <div
                    ...
                    aria-modal="true"
                    on:keydown={(e) => {
                      if (e.key === "Escape") showQrCodeModal = false;
                    }}
                  >
                    <h3 class="text-lg font-semibold mb-4">
                      {$t("wallet.qrModal.title")}
                    </h3>

                    <img
                      src={qrCodeDataUrl}
                      alt={$t("wallet.qrModal.alt")}
          ...
          <div class="mt-4">
            ...
            <div class="flex gap-2 mt-1">
              <Input
                type="text"
                value={privateKeyVisible
                  ? $etcAccount.private_key
                  : "•".repeat($etcAccount.private_key.length)}
                readonly
            ...
            <p class="text-xs text-muted-foreground mt-1">
              {$t("warnings.neverSharePrivateKey")}
            </p>
          </div>
          ...
          <div class="mt-6 space-y-2">
            ...
            {#if exportMessage}<p
                class="text-xs text-center mt-2 {exportMessage.includes(
                  'successfully',
                )
                  ? 'text-green-600'
                  : 'text-red-600'}"
      ...
      <Card class="p-6">
        ...
        <form autocomplete="off" data-form-type="other" data-lpignore="true">
          <div class="space-y-4">
            <div>
                ...
                >
                  <svg
                    ...
                    ><rect x="3" y="3" width="7" height="7"></rect><rect
                      ...
                      width="7"
                      height="7"
                    ></rect><rect x="3" y="14" width="7" height="7"></rect><line
                      x1="14"
                      x2="14"
                      y1="14"
                      y2="21"
                    ></line><line x1="21" x2="21" y1="14" y2="21"></line><line
                      x1="21"
                      x2="14"
                      y1="21"
                      y2="21"
                ...
                >
                  ...
                  >
                    <h3 class="text-lg font-semibold mb-4 text-center">
                      {$t("transfer.recipient.scanQrTitle")}
                    </h3>
              ...
              <div class="flex items-center justify-between mt-1">
                <span class="text-xs text-muted-foreground">
                  {recipientAddress.length}/42 {$t(
                    "transfer.recipient.characters",
                  )}
                  {#if recipientAddress.length <= 42}
                    ({42 - recipientAddress.length}
                    {$t("transfer.recipient.remaining")})
                  {:else}
                    ({recipientAddress.length - 42}
                    {$t("transfer.recipient.over")})
                  {/if}
                </span>
                {#if addressWarning}
            ...
            <div>
              ...
              <div class="flex items-center justify-between mt-1">
                <p class="text-xs text-muted-foreground">
                  {$t("transfer.available", {
                    values: { amount: $wallet.balance.toFixed(2) },
                  })}
                </p>
              ...
              <div class="mt-3">
                <div class="inline-flex rounded-md border overflow-hidden">
                  <button
                    type="button"
                    class="px-3 py-1 text-xs {feePreset === 'low'
                  ...
                  <button
                    type="button"
                    class="px-3 py-1 text-xs border-l {feePreset === 'market'
                      ? 'bg-foreground text-background'
                      : 'bg-background'}"
                    on:click={() => (feePreset = "market")}
                    >{$t("fees.market")}</button
                  >
                  <button
                    type="button"
                    class="px-3 py-1 text-xs border-l {feePreset === 'fast'
                      ? 'bg-foreground text-background'
                      : 'bg-background'}"
                ...
                <p class="text-xs text-muted-foreground mt-2">
                  {$t("fees.estimated")}: {estimatedFeeDisplay}
                </p>
              </div>
            </div>
            ...
            <Button
              type="button"
              class="w-full justify-center bg-gray-100 hover:bg-gray-200 text-gray-800 rounded transition-colors py-2 font-normal"
              on:click={() => (showPending = !showPending)}
              aria-label={$t("transfer.viewPending")}
            >
            ...
            {#if showPending}
              <div class="mt-2 p-3 bg-gray-50 rounded shadow">
                <h3 class="text-sm mb-2 text-gray-700 font-normal">
                  {$t("transfer.pending.title")}
                </h3>
                <ul class="space-y-1">
                  {#each $transactions.filter((tx) => tx.status === "pending") as tx}
                    <li class="text-xs text-gray-800 font-normal">
                      {tx.description} ({tx.type === "sent"
                        ? $t("transactions.item.to")
                        : $t("transactions.item.from")}: {tx.type === "sent"
                        ? tx.to
                        : tx.from}) - {tx.amount} Chiral
                    </li>
                  {:else}
                    <li class="text-xs text-gray-500 font-normal">
                      {$t("transfer.pending.noDetails")}
                    </li>
                  {/each}
                </ul>
              </div>
      ...
      <Card class="p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex gap-2">
            ...
            <Button variant="outline" on:click={openImportMnemonic}
              >Import</Button
            >
          </div>
        </div>
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-muted-foreground">
                  {tx.type === "received"
                    ? $t("transactions.item.from")
                    : $t("transactions.item.to")}: {tx.type === "received"
                    ? tx.from
                    : tx.to}
                </p>
              </div>
            ...
            <div class="text-right">
              <p
                class="text-sm font-medium {tx.type === 'received'
                  ? 'text-green-600'
                  : 'text-red-600'}"
              >
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-green-700">
                  {$t("security.2fa.enabledDesc")}
                </p>
              </div>
            </div>
            <Button variant="destructive" on:click={disable2FA}
              >{$t("security.2fa.disable")}</Button
            >
          </div>
        {:else}
          <div
            class="flex items-center justify-between p-4 border-2 border-dashed rounded-lg"
          >
            <p class="text-sm text-muted-foreground">
              {$t("security.2fa.disabledDesc")}
            </p>
            <Button on:click={setup2FA}>{$t("security.2fa.enable")}</Button>
          </div>
        {/if}
        <p class="text-xs text-muted-foreground">
          {$t("security.2fa.explanation")}
        </p>
      </div>
    </Card>
  {/if}
  ...
  <Card class="p-6">
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center gap-2">
        <div class="flex-1">
          <Input
            ...
            class="w-full {passwordStrength
              ? `border-${passwordStrength === 'strong' ? 'green' : passwordStrength === 'medium' ? 'yellow' : 'red'}-500`
              : ''}"
          ...
          {#if keystorePassword}
            <div class="mt-1 flex items-center gap-2">
              <div class="h-1 flex-1 bg-gray-200 rounded-full overflow-hidden">
                <div
                  class="h-full transition-all duration-300 {passwordStrength ===
                  'strong'
                    ? 'bg-green-500 w-full'
                    : passwordStrength === 'medium'
                      ? 'bg-yellow-500 w-2/3'
              ...
              <span
                class="text-xs {passwordStrength === 'strong'
                  ? 'text-green-600'
                  : passwordStrength === 'medium'
                    ? 'text-yellow-600'
                    : 'text-red-600'}"
            ...
            <ul class="text-xs text-muted-foreground mt-2 space-y-1">
              <li class={keystorePassword.length >= 8 ? "text-green-600" : ""}>
                • {$t("password.requirements.length")}
              </li>
              <li
                class={/[A-Z]/.test(keystorePassword) ? "text-green-600" : ""}
              >
              ...
              <li
                class={/[0-9]/.test(keystorePassword) ? "text-green-600" : ""}
              >
                • {$t("password.requirements.number")}
              </li>
              <li
                class={/[!@#$%^&*(),.?":{}|<>]/.test(keystorePassword)
                  ? "text-green-600"
                  : ""}
              >
      ...
      {#if keystoreSaveMessage}
        <p
          class="text-xs text-center mt-2 {keystoreSaveMessage
            .toLowerCase()
            .includes('success')
            ? 'text-green-600'
            : 'text-red-600'}"
    ...
    <div class="space-y-6">
        ...
        <div class="space-y-4">
          <div>
            ...
            <Input
              ...
              class="mt-2 font-mono text-sm {isBlacklistAddressValid
                ? 'border-green-300'
                : ''}"
            />
            <div class="flex items-center justify-between mt-1">
              <span class="text-xs text-muted-foreground">
                {newBlacklistEntry.chiral_address.length}/42 {$t(
                  "transfer.recipient.characters",
                )}
                {#if newBlacklistEntry.chiral_address.length <= 42}
                  ({42 - newBlacklistEntry.chiral_address.length}
                  {$t("transfer.recipient.remaining")})
                {:else}
          ...
          <div>
            ...
            {#if newBlacklistEntry.reason.length > 150}
              <p class="text-xs text-orange-500 mt-1">
                {$t("blacklist.add.remaining", {
                  values: { remaining: 200 - newBlacklistEntry.reason.length },
                })}
              </p>
            {/if}
          </div>
          ...
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-xs text-muted-foreground"
              >{$t("blacklist.quickReasons.label")}</span
            >
      ...
      <div>
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-2">
            <div class="relative">
              ...
              {:else}
                <div
                  class="absolute right-2 top-1/2 transform -translate-y-1/2 text-muted-foreground pointer-events-none"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
        ...
        {#if filteredBlacklist.length === 0 && $blacklist.length === 0}
          <div
            class="text-center py-8 text-muted-foreground border-2 border-dashed border-gray-200 rounded-lg"
          >
        ...
        {:else if filteredBlacklist.length === 0 && blacklistSearch}
          <div class="text-center py-6 text-muted-foreground">
            <p>
              {$t("blacklist.list.noMatch", { values: { q: blacklistSearch } })}
            </p>
          </div>
        {:else}
          <div class="space-y-2 max-h-64 overflow-y-auto">
            {#each filteredBlacklist as entry, index (entry.chiral_address)}
              <div
                class="flex items-center justify-between p-3 bg-red-50 border border-red-100 rounded-lg group hover:bg-red-100/50 transition-colors"
              >
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-muted-foreground">
                    {$t("blacklist.list.addedAt", {
                      values: { date: formatDate(entry.timestamp) },
                    })}
                  </p>
                </div>
          ...
          {#if $blacklist.length > 5}
            <div class="text-center mt-3">
              <p class="text-xs text-muted-foreground">
                {$t("blacklist.list.showing", {
                  values: {
                    shown: filteredBlacklist.length,
        ...
        <div class="flex gap-2">
          {#if $blacklist.length > 0}
            <Button
              ...
              title={$t("blacklist.actions.exportTitle", {
                values: { count: $blacklist.length },
              })}
            >
              {$t("blacklist.actions.export")}
              {$blacklist.length > 0 ? `(${$blacklist.length})` : ""}
            </Button>
          {/if}
          <Button
            variant="outline"
  ...
  {#if show2faSetupModal && totpSetupInfo}
    <div
      ...
      on:keydown={(e) => {
        if (e.key === "Enter" || e.key === " ") show2faSetupModal = false;
      }}
    >
      <div
        ...
        aria-modal="true"
        on:keydown={(e) => {
          if (e.key === "Escape") show2faSetupModal = false;
        }}
      >
        <h3 class="text-xl font-semibold mb-2">
          {$t("security.2fa.setup.title")}
        </h3>
        <p class="text-sm text-muted-foreground mb-4">
          {$t("security.2fa.setup.step1")}
        </p>

        <div
          class="flex flex-col md:flex-row gap-4 items-center bg-background p-4 rounded-lg"
        >
          <img
            src={totpSetupInfo.qrCodeDataUrl}
            alt="2FA QR Code"
            class="w-40 h-40 rounded-md border bg-white p-1"
          />
          <div class="space-y-2">
            <p class="text-sm">{$t("security.2fa.setup.scanAlt")}</p>
            <p class="text-xs text-muted-foreground">
              {$t("security.2fa.setup.manualLabel")}
            </p>
            <div class="flex items-center gap-2 bg-secondary p-2 rounded">
              <code class="text-sm font-mono break-all"
                >{totpSetupInfo.secret}</code
              >
              <Button
                size="icon"
                variant="ghost"
        ...
        <div class="mt-6 flex justify-end gap-2">
          <Button variant="outline" on:click={() => (show2faSetupModal = false)}
            >{$t("actions.cancel")}</Button
          >
          <Button
            on:click={verifyAndEnable2FA}
            disabled={isVerifying2fa || totpVerificationCode.length < 6}
          >
            {isVerifying2fa
              ? $t("actions.verifying")
  ...
  {#if show2faPromptModal}
    ...
    >
      ...
      >
        <h3 class="text-xl font-semibold mb-2">
          {$t("security.2fa.prompt.title")}
        </h3>
        <p class="text-sm text-muted-foreground mb-4">
          {$t("security.2fa.prompt.subtitle")}
        </p>
        ...
        <div class="mt-6 flex justify-end gap-2">
          ...
          <Button
            ...
            disabled={isVerifyingAction || totpActionCode.length < 6}
          >
            {isVerifyingAction
              ? $t("actions.verifying")
              : $t("actions.confirm")}
  ...
  {#if autoLockMessage}
    <div
      class="fixed top-0 left-0 w-full bg-yellow-100 text-yellow-800 text-center py-2 z-50"
    >
      {autoLockMessage}
    </div>
  {/if}
