...
>
 <div class="bg-white/[0.05] rounded-xl border border-white/[0.06] w-full max-w-md overflow-hidden">
 <!-- Header -->
 ...
 <h3 class="text-base font-semibold text-white/90">Select from Drive</h3>
 </div>
 <button
 onclick={onClose}
 ...
 <span class="text-sm text-white/50">Loading Drive files...</span>
 </div>
 {:else if files.length === 0}
 <div class="flex flex-col items-center justify-center py-16">
 <div class="flex h-14 w-14 items-center justify-center rounded-xl bg-white/[0.05] mb-3">
 <HardDrive class="w-7 h-7 text-white/50 opacity-40" />
 </div>
 <p class="text-sm font-medium text-white/40">No files in Drive</p>
 <p class="text-xs text-white/50 mt-1">Upload files in the Drive page first</p>
 </div>
 {:else}
 <!-- Select all -->
 <div class="px-4 py-2.5 border-b border-white/[0.06] bg-white/[0.05]">
 <button
 onclick={toggleAll}
 class="flex items-center gap-2 text-xs font-medium text-white/40 hover:text-white/50 transition-colors
 focus:outline-none focus:underline"
 >
 <div class="w-4.5 h-4.5 rounded border-2 flex items-center justify-center flex-shrink-0
 {selected.size === files.length && files.length > 0 ?'border-violet-500 bg-violet-500' :'border-white/[0.06]'}">
 ...
 {selected.has(file.id) ?'bg-violet-950/20/50' :'hover:bg-white/[0.05]/30'}
 focus:outline-none focus:bg-white/[0.3]/30"
 >
 <div class="flex items-center gap-2.5 min-w-0">
 <div class="w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors
 {selected.has(file.id) ?'border-violet-500 bg-violet-500' :'border-white/[0.06]'}">
 ...
 <span class="text-xs text-white/50 ml-2 flex-shrink-0 tabular-nums">{formatBytes(file.size)}</span>
 </button>
 {/each}
 </div>

 <!-- Footer -->
 <div class="flex items-center justify-between p-4 border-t border-white/[0.06] bg-white/[0.05]">
 ...
 <button
 onclick={onClose}
 class="px-3 py-1.5 text-sm font-medium text-white/50 border border-white/[0.06] rounded-lg
