...
>
  <div class="backdrop-blur-2xl bg-white/15 dark:bg-white/10 rounded-2xl shadow-2xl border border-white/20 dark:border-white/10 w-full max-w-md overflow-hidden">
    <!-- Header -->
    <div class="flex items-center justify-between p-5 pb-4 border-b border-white/15 dark:border-white/10">
      <div class="flex items-center gap-2.5">
        <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-primary-500/15 dark:bg-primary-500/10">
    ...
    {:else}
      <!-- Select all -->
      <div class="px-4 py-2.5 border-b border-white/15 dark:border-white/10 bg-white/8 dark:bg-white/5">
        <button
          onclick={toggleAll}
        ...
        >
          <div class="w-4.5 h-4.5 rounded border-2 flex items-center justify-center flex-shrink-0
            {selected.size === files.length && files.length > 0 ? 'border-primary-500 bg-primary-500' : 'border-white/20 dark:border-white/10'}">
      ...
      <!-- File list -->
      <div class="max-h-72 overflow-y-auto divide-y divide-white/10 dark:divide-white/5">
        {#each files as file (file.id)}
          <button
            onclick={() => toggleFile(file.id)}
            class="flex items-center justify-between w-full px-4 py-3 text-left text-sm transition-colors
              {selected.has(file.id) ? 'bg-primary-50/50 dark:bg-primary-900/10' : 'hover:bg-white/10 dark:hover:bg-white/5/30'}
              focus:outline-none focus:bg-gray-50 dark:focus:bg-white/5/30"
          >
            <div class="flex items-center gap-2.5 min-w-0">
              <div class="w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors
                {selected.has(file.id) ? 'border-primary-500 bg-primary-500' : 'border-white/20 dark:border-white/10'}">
      ...
      <!-- Footer -->
      <div class="flex items-center justify-between p-4 border-t border-white/15 dark:border-white/10 bg-white/8 dark:bg-white/5">
        ...
        <div class="flex gap-2">
          <button
            onclick={onClose}
            class="px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 border border-white/20 dark:border-white/10 rounded-lg
          ...
          <button
            ...
            class="px-3.5 py-1.5 text-sm font-semibold backdrop-blur-md bg-primary-500/80 dark:bg-primary-600/70 border border-primary-400/30 hover:bg-primary-500/90 dark:hover:bg-primary-600/80 text-white rounded-lg transition-colors
