...
>
 <div class="cyber-panel w-full max-w-lg overflow-hidden">
 <!-- Header -->
 ...
 <p class="text-xs text-white/50 mt-0.5 font-mono">
 ...
 <div class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-white/[0.04]">
 ...
 class="w-full p-3 text-sm font-mono bg-white/[0.05] border border-gray-800/60 rounded-xl text-white placeholder:text-white/30
 focus:ring-blue-400/20/20 focus:border-violet-400 focus:outline-none resize-none transition-all"
 ></textarea>

 {#if showDrivePicker}
 <div class="mt-2 max-h-40 overflow-y-auto rounded-xl border border-gray-800/60 bg-white/[0.04] divide-y divide-white/[0.06]">
 {#if driveFiles.length === 0}
 <p class="text-xs text-white/40 p-4 text-center">No files in Drive</p>
 {:else}
 {#each driveFiles as file (file.id)}
 <button
 onclick={() => onAddDriveFile(file.id, file.name)}
 disabled={publishingDriveFile === file.id}
 class="flex items-center justify-between w-full px-3 py-2.5 text-left text-sm hover:bg-white/[0.05]/50 transition-colors
 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:bg-white/[0.06]"
 >
 ...
 <!-- Cost summary -->
 <div class="p-4 rounded-xl bg-blue-500/[0.06]/50 border border-gray-800/60">
 ...
 <!-- Footer actions -->
 <div class="flex justify-end gap-3 p-5 pt-4 border-t border-gray-800/60 bg-white/[0.04]">
 <button
 onclick={onClose}
 class="px-4 py-2.5 text-sm font-medium text-white/70 border border-gray-800/60 rounded-xl
