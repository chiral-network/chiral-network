...
>
 ...
 <div class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-[var(--surface-1)]">
 ...
 class="w-full p-3 text-sm font-mono bg-[var(--surface-1)] border border-[var(--border)]/60 rounded-xl text-white placeholder:text-[var(--text-tertiary)]
 focus:ring-blue-400/20/20 focus:border-violet-400 focus:outline-none resize-none transition-all"
 ></textarea>

 {#if showDrivePicker}
 <div class="mt-2 max-h-40 overflow-y-auto rounded-xl border border-[var(--border)]/60 bg-[var(--surface-1)] divide-y divide-white/[0.06]">
 {#if driveFiles.length === 0}
 <p class="text-xs text-[var(--text-tertiary)] p-4 text-center">No files in Drive</p>
 {:else}
 {#each driveFiles as file (file.id)}
 <button
 onclick={() => onAddDriveFile(file.id, file.name)}
 disabled={publishingDriveFile === file.id}
 class="flex items-center justify-between w-full px-3 py-2.5 text-left text-sm hover:bg-[var(--surface-1)]/50 transition-colors
 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:bg-[var(--surface-1)]"
 >
 ...
 />
 <div class="flex justify-between text-[10px] text-[var(--text-tertiary)] mt-1 tabular-nums">
 <span>1 day</span>
 <span>1 year</span>
 </div>
 </div>
 ...
 <!-- Cost summary -->
 <div class="p-4 rounded-xl bg-blue-500/[0.06]/50 border border-[var(--border)]/60">
 ...
 <!-- Footer actions -->
 <div class="flex justify-end gap-3 p-5 pt-4 border-t border-[var(--border)]/60 bg-[var(--surface-1)]">
 <button
 onclick={onClose}
 class="px-4 py-2.5 text-sm font-medium text-white/70 border border-[var(--border)]/60 rounded-xl
