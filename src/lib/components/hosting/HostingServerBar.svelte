...
<div class="rounded-2xl border p-4 transition-colors
  {serverRunning
    ...
    : 'border-cyan-500/15 bg-gray-900'}
  shadow-[0_0_10px_rgba(6,182,212,0.05)] backdrop-blur">
  <div class="flex items-center justify-between gap-4 flex-wrap">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        {:else}
          <p class="text-xs text-gray-500 mt-0.5">Server is stopped. Start it to host content.</p>
        {/if}
      </div>
    </div>
    ...
    <div class="flex items-center gap-3">
      {#if !serverRunning}
        <div class="flex items-center gap-2">
          <label for="hosting-port" class="text-xs text-gray-500 font-medium">Port</label>
          <input
            id="hosting-port"
            type="number"
            value={port}
            ...
            min="1024"
            max="65535"
            class="w-20 rounded-lg border border-cyan-500/20 bg-gray-900 px-2.5 py-1.5 text-sm text-gray-100 text-center tabular-nums
      ...
      {#if serverRunning}
        <button
          ...
          class="flex items-center gap-2 rounded-xl bg-red-500/100 px-4 py-2 text-sm font-medium text-white transition-colors
            hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:ring-offset-2"
        >
      ...
      {:else}
        <button
          ...
          class="flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-2 text-sm font-medium text-white transition-colors
            hover:bg-cyan-600/80 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:ring-offset-2
