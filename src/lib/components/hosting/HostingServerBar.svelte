...
<div class="rounded-xl border p-4 transition-colors
 ...
 :'border-gray-800/60 bg-white/[0.07] backdrop-blur-xl'}
  backdrop-blur">
 <div class="flex items-center justify-between gap-4 flex-wrap">
 <div class="flex items-center gap-3">
 <!-- Status indicator -->
 <div class="relative flex h-10 w-10 items-center justify-center rounded-xl
 {serverRunning ?'bg-green-500/[0.08]' :'bg-white/[0.05]'}">
 <Server class="h-5 w-5 {serverRunning ?'text-green-400' :'text-white/40'}" />
 {#if serverRunning}
 ...
 min="1024"
 max="65535"
 class="w-20 rounded-lg border border-white/[0.08] bg-gray-950 px-2.5 py-1.5 text-sm text-white text-center tabular-nums
 focus:border-violet-400 focus:outline-none focus:ring-blue-400/20/30
"
 />
 </div>
 {/if}
