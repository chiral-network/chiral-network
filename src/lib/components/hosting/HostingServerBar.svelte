...
<div class="rounded-xl border p-4 transition-colors
 ...
 {serverRunning ?'bg-green-500/15' :'bg-white/[0.05]'}">
 ...
 Local HTTP Server
 {#if serverRunning}
 <span class="inline-flex items-center gap-1 rounded-full bg-green-500/[0.15] px-2 py-0.5 text-[10px] font-medium text-green-400 uppercase tracking-wide">
 Online
 </span>
 {/if}
 </h2>
 ...
 {:else}
 <p class="text-xs text-white/40 mt-0.5">Server is stopped. Start it to host content.</p>
 {/if}
 </div>
 </div>
 ...
 <label for="hosting-port" class="text-xs text-white/40 font-medium">Port</label>
 <input
 id="hosting-port"
 type="number"
 value={port}
 ...
 min="1024"
 max="65535"
 class="w-20 rounded-lg border border-white/[0.06] bg-white/[0.05] px-2.5 py-1.5 text-sm text-white/90 text-center tabular-nums
