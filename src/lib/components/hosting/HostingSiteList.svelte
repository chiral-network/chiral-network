...
<div class="rounded-2xl border border-gray-200 dark:border-gray-800/60 bg-white dark:bg-gray-950 p-5  backblur-sm">
  <h2 class="mb-4 text-base font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
    ...
    Hosted Sites
    {#if sites.length > 0}
      <span class="rounded-full bg-primary-100 px-2 py-0.5 text-xs font-medium text-cyan-700 dark:text-cyan-300">
        {sites.length}
      </span>
    {/if}
  </h2>
  ...
  {:else}
    <div class="space-y-3">
      {#each sites as site (site.id)}
        <div class="group rounded-xl border border-gray-200 dark:border-gray-800/40 bg-gray-100 dark:bg-gray-800 p-4 transition-all
          hover:border-gray-200 dark:border-gray-800/50 hover:
         /50">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                <h3 class="font-semibold text-gray-900 dark:text-gray-100">{site.name}</h3>
                {#if site.relayUrl}
                ...
                {:else}
                  <span class="inline-flex items-center rounded-full bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-[10px] font-medium text-gray-500 text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                    Local
                  </span>
                {/if}
              </div>
              ...
              <p class="mt-1 font-mono text-xs truncate
                {site.relayUrl ? 'text-emerald-600 dark:text-emerald-400' : 'text-cyan-600 dark:text-cyan-400'}">
              ...
              <div class="mt-2 flex items-center gap-3 text-xs text-gray-500">
                ...
                <span aria-hidden="true" class="text-gray-600 dark:text-gray-300">|</span>
              ...
              {#if site.files.length > 0}
                <div class="mt-2.5 flex flex-wrap gap-1.5">
                  {#each site.files.slice(0, 5) as file}
                    <span class="rounded-md bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-[10px] font-medium text-gray-500/50 text-gray-500 dark:text-gray-400">
                      {file.path}
                    </span>
                  {/each}
                  {#if site.files.length > 5}
                    <span class="rounded-md bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-[10px] font-medium text-gray-500 dark:text-gray-400/50">
            ...
            <div class="flex items-center gap-1 flex-shrink-0 opacity-70 group-hover:opacity-100 transition-opacity">
              {#if site.relayUrl}
                ...
                >
                  {#if publishingStates[site.id]}
                    <div class="h-4 w-4 animate-spin rounded-full border-2 border-gray-200 dark:border-gray-800/50 border-t-orange-500"></div>
                  {:else}
              ...
              {:else}
                <button
                  ...
                  class="rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-emerald-500/10 hover:text-green-500
                    focus:outline-none focus:ring-2 focus:ring-green-400/30 disabled:opacity-50"
                >
                  {#if publishingStates[site.id]}
                    <div class="h-4 w-4 animate-spin rounded-full border-2 border-gray-200 dark:border-gray-800/50 border-t-green-500"></div>
                  {:else}
              ...
              <button
                ...
                class="rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-white/[0.03] hover:text-gray-500 dark:text-gray-400
              ...
              <button
                ...
                class="rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-100 dark:hover:bg-white/[0.03] hover:text-cyan-600 dark:text-cyan-400
              ...
              <button
                ...
                class="rounded-lg p-2 text-gray-500 dark:text-gray-400 transition-colors hover:bg-red-500/10 hover:text-red-500
