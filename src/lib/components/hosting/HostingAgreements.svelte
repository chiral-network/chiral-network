...
{#if !loadingAgreements && incomingProposals.length > 0}
  <div class="rounded-2xl border-2 border-gray-200 dark:border-gray-800/50 bg-cyan-50 dark:bg-cyan-500/[0.06] p-5 ">
    <div class="flex items-center gap-3 mb-4">
      <div class="relative flex h-9 w-9 items-center justify-center rounded-xl bg-blue-500/10">
        <Send class="w-4.5 h-4.5 text-cyan-600 dark:text-cyan-400" />
      ...
      <div>
        <h2 class="font-semibold text-base text-gray-900 dark:text-gray-100">Incoming Proposals</h2>
    ...
    <div class="space-y-2.5">
      {#each incomingProposals as proposal (proposal.agreementId)}
        <div class="flex items-center justify-between gap-3 p-4 rounded-xl bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800/60">
          <div class="min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-sm font-semibold text-gray-900 dark:text-gray-100 font-mono">
                {formatPeerId(proposal.clientPeerId)}
              </span>
              <span class="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              ...
              class="flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium bg-red-500/10 hover:bg-red-500/10 text-red-600 dark:text-red-400 rounded-lg transition-colors
...
<div class="rounded-2xl border border-gray-200 dark:border-gray-800/60 bg-white dark:bg-gray-950  backblur-sm overflow-hidden">
  <button
    ...
    class="flex items-center justify-between w-full p-5 hover:bg-gray-100 dark:hover:bg-white/[0.02] transition-colors
  ...
  >
    <div class="flex items-center gap-3">
      ...
      <div class="text-left">
        <h2 class="font-semibold text-base text-gray-900 dark:text-gray-100">My Agreements</h2>
        <p class="text-xs text-gray-500 mt-0.5">
    ...
    <div class="flex items-center gap-2">
      {#if activeAgreements.filter(a => a.status === 'active').length > 0}
        <span class="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">
  ...
  {#if showAgreements}
    <div class="border-t border-gray-200 dark:border-gray-800/40 p-5 pt-4">
      {#if loadingAgreements}
        <div class="flex flex-col items-center justify-center py-12">
          <Loader2 class="w-6 h-6 text-gray-500 dark:text-gray-400 animate-spin mb-3" />
          <span class="text-sm text-gray-500 dark:text-gray-400">Loading agreements...</span>
        </div>
          ...
          {#each activeAgreements as agreement (agreement.agreementId)}
            ...
            <div class="p-4 rounded-xl border transition-all
              {hasCancelRequest && agreement.cancelRequestedBy !== myPeerId
                ...
                : 'border-gray-100 bg-gray-100 dark:bg-gray-800 hover:border-gray-200 dark:border-gray-800/50'}">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  ...
                  <div class="flex items-center gap-3 text-xs text-gray-500 mt-2">
                    <span class="flex items-center gap-1">
                      <FileText class="w-3 h-3 text-gray-500 dark:text-gray-400" />
                      {agreement.fileHashes.length} file{agreement.fileHashes.length !== 1 ? 's' : ''}
                    </span>
                    <span class="flex items-center gap-1">
                      <Clock class="w-3 h-3 text-gray-500 dark:text-gray-400" />
                    ...
                    <span class="flex items-center gap-1">
                      <Coins class="w-3 h-3 text-gray-500 dark:text-gray-400" />
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                  {#if agreement.cancelRequestedBy && agreement.cancelRequestedBy !== myPeerId}
                    ...
                    <div class="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        class="text-xs font-medium px-3 py-1.5 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-800/60 rounded-lg
                          hover:bg-gray-100 dark:hover:bg-white/[0.02] transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
                      >
                        Deny
                      </button>
                    </div>
                  ...
                  {:else if agreement.status === 'proposed' && isClient}
                    <button
                      ...
                      class="text-xs font-medium px-3 py-1.5 text-red-600 dark:text-red-400 border border-red-500/20 rounded-lg
...
{#if !loadingAgreements && hostedFiles.length > 0}
  <div class="rounded-2xl border border-gray-200 dark:border-gray-800/60 bg-white dark:bg-gray-950 p-5  backblur-sm">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h2 class="font-semibold text-base text-gray-900 dark:text-gray-100">Files I'm Hosting</h2>
        <p class="text-xs text-gray-500 mt-0.5">
    ...
    <div class="space-y-2 rounded-xl border border-gray-200 dark:border-gray-800/40 divide-y divide-cyan-500/10 overflow-hidden">
      {#each hostedFiles as file (file.fileHash + file.agreementId)}
        <div class="flex items-center justify-between p-3.5 hover:bg-gray-100 dark:hover:bg-white/[0.02] transition-colors">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <p class="text-sm font-mono text-gray-600 dark:text-gray-300 truncate">
                {file.fileHash}
              </p>
