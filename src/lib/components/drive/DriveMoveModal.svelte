...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onclick={onClose}>
  <div
    class="bg-white dark:bg-gray-950 rounded-xl border border-gray-200 dark:border-gray-800/60 w-full max-w-sm mx-4 p-6"
    onclick={(e) => e.stopPropagation()}
  >
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Move "{item.name}"</h3>
      <button onclick={onClose} class="p-1 hover:bg-gray-100 dark:hover:bg-white/[0.03] rounded-lg transition">
        <X class="w-5 h-5 text-gray-500" />
      </button>
    </div>

    <div class="max-h-64 overflow-y-auto border border-gray-200 dark:border-gray-800/60 rounded-lg">
      <!-- Root -->
      <button
        onclick={() => selectedFolderId = null}
        class="flex items-center gap-2 w-full px-3 py-2.5 text-sm transition
          {selectedFolderId === null
            ? 'bg-cyan-50 dark:bg-cyan-500/[0.06] text-blue-600 dark:text-blue-400'
            : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-800'}"
      >
    ...
    <div class="flex justify-end gap-2 mt-4">
      <button
        onclick={onClose}
        class="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/[0.03] rounded-lg transition"
      >
        Cancel
      </button>
      <button
