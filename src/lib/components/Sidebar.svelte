...
<div class="md:hidden sticky top-0 z-50 flex items-center justify-between h-14 px-3 bg-[var(--surface-1)] border-b border-[var(--border)]">
 ...
 <span class="text-sm font-semibold text-[var(--text-primary)]">Chiral</span>
 </div>
 <div class="flex items-center gap-2">
...
<!-- Mobile menu overlay -->
{#if mobileOpen}
 <div class="md:hidden fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onclick={() => mobileOpen = false}></div>
...
{/if}
<!-- Desktop sidebar -->
<aside class="hidden md:flex fixed top-0 left-0 z-40 h-screen flex-col bg-[var(--surface-1)] border-r border-[var(--border)] transition-[width] duration-200 {collapsed ?'w-16' :'w-48'}">
 <div class="flex items-center h-14 px-3 border-b border-[var(--border)] shrink-0 overflow-hidden {collapsed ?'justify-center' :'justify-between'}">
 <div class="flex items-center gap-2 overflow-hidden">
 <img src="/logo.png" alt="Chiral Network" class="w-7 h-7 rounded-lg shrink-0" />
 {#if !collapsed}<span class="text-sm font-semibold text-[var(--text-primary)] whitespace-nowrap">Chiral Network</span>{/if}
 </div>
 {#if !collapsed}<button onclick={toggleCollapse} class="p-1 text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] hover:bg-[var(--surface-2)] rounded-md transition shrink-0" title="Collapse sidebar"><ChevronLeft class="w-4 h-4" /></button>{/if}
 </div>
