...
impl Default for FileConfig {
    fn default() -> Self {
        Self {
            default_download_dir: dirs::download_dir().unwrap_or_else(|| PathBuf::from("./downloads")),
            completed_dir: None,
            incomplete_dir: None,
            auto_manage: true,
...
impl BitTorrentConfigManager {
    ...
    pub async fn new(app_handle: AppHandle) -> Result<Self> {
        ...
        // Create store for persistent configuration
        let store = StoreBuilder::new(app_handle, "bittorrent-config.json".parse()?)
            .build()
            .await?;
    ...
    fn validate_file_config(&self, config: &FileConfig) -> Result<()> {
        if !config.default_download_dir.exists() {
            return Err(anyhow!("Default download directory does not exist: {}", 
                config.default_download_dir.display()));
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_config_validation() {
        let manager = BitTorrentConfigManager {
            store: unsafe { std::mem::zeroed() }, // Mock for testing
            config: BitTorrentConfig::default(),
        };

        // Test invalid port
        let mut invalid_config = BitTorrentConfig::default();
        invalid_config.network.listening_port = 70000;
        
        assert!(manager.validate_config(&invalid_config).is_err());

        // Test invalid timeout
        let mut invalid_config = BitTorrentConfig::default();
        invalid_config.peers.connection_timeout = 0;
