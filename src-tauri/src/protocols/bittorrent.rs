...
use bendy::serde::{from_bytes, to_bytes, Error as BendyError};
...
pub fn parse_torrent_file(file_path: &str) -> Result<TorrentMetadata, String> {
    ...
    // Parse as a generic bencode value first to extract the raw info dictionary
    let value: Value = from_bytes(&torrent_bytes)
        .map_err(|e: BendyError| format!("Failed to parse .torrent file: {}", e))?;

    // Extract the info dictionary from the parsed value
    let info_dict = match &value {
        Value::Dict(dict) => {
            dict.get(&b"info"[..])
                .ok_or("Missing 'info' dictionary in torrent file")?
        }
        _ => return Err("Torrent file is not a dictionary".to_string()),
    };

    // Encode the info dictionary back to bytes for hashing
    let info_bytes = to_bytes(info_dict)
    ...
    let mut metadata: TorrentMetadata = from_bytes(&torrent_bytes)
        .map_err(|e: BendyError| format!("Failed to parse .torrent file: {}", e))?;
