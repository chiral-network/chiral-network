...
| ID | Title | File | Status |
|----|-------|------|--------|
| FM-A07 | `FileMetadata::publisher_signature` is documented as enforced but every production publisher writes the empty string and `search_file` consumes the metadata regardless of `verify_publisher()` | `lib.rs:1180-1199, ~393, 2511, 5267, 5397, 6677` and `lib.rs:1883-1888` | **Filed** — the writer side is missing across many call sites; needs a coordinated wiring pass. **Follow-up.** |
| FM-A08 | `SeederInfo.signature` documented as "prevents payment redirection" but production publishers write empty signatures and `fetch_seeders` accepts empty-signature entries | `lib.rs:1281-1308, 1129-1148` and writer sites | **Filed** — same as FM-A07. **Follow-up.** |
...
| FM-A14 | CORS layer is `allow_origin(Any).allow_methods(Any).allow_headers(Any)` on mutating routes; combined with FM-A03 lets any visited webpage CSRF the local daemon | `hosting_server.rs:516-522` | **Filed** — tighten CORS to known origins (or block credentialed cross-origin on mutating verbs). **Follow-up.** |
| FM-A15 | Drive multipart upload has no `DefaultBodyLimit`; 499 MB allocation is realised before the 500 MB check rejects | `drive_api.rs:353-403, 1469-1485` | **Filed** — add `DefaultBodyLimit::max(500 * 1024 * 1024)` to the Drive router (CDN router already has it). **Follow-up.** |
| FM-A16 | `is_item_under_shared_root` walks `parent_id` upward without a visited-set; a parent-cycle hangs the request thread holding the manifest read lock | `drive_api.rs:186-201` | **Filed** — add depth bound or visited-set. **Follow-up.** |
| FM-A17 | `wallet::verify_tx_details` does not compare tx `chainId` to the chiral chain id; a cross-chain replayed signed-tx with same address pair would pass | `wallet.rs:639-661` | **Filed** — add `chain_id` check. **Follow-up.** |
| FM-A18 | `recover_signer` does not enforce low-`s` (EIP-2); both `(r, s)` and `(r, n-s)` recover the same pubkey, so any future caller treating the hex sig as a dedup key is fooled | `wallet.rs:565-592` | **Filed** — enforce low-s. Today no caller uses sig hex as a dedup key, but DHT replay/dedup logic can grow into one. **Follow-up.** |
...
| FM-A20 | `FolderManifest` is read in `search_folder` without verifying its publisher signature; `name` and `owner_wallet` are not bound to the content-addressed folder hash | `lib.rs:5917-5969, 5539-5553` | **Filed** — verify on read. **Follow-up.** |
| FM-A21 | Spent-tx guard for paid downloads does not bind the tx to a `file_hash`; a single payment to wallet W can redeem any file priced ≤ V from any seeder advertising W (multi-seeder shared-wallet collusion) | `dht.rs:1051-1102, 3618-3651` | **Filed** — bind ledger entries to `(tx, file_hash)` pairs. **Follow-up.** |

### Low — theoretical / minor

| ID | Title | File | Status |
|----|-------|------|--------|
| FM-A22 | Chunk position not verified against requested index; final SHA-256 catches the discrepancy so it's a bandwidth-waste DoS, not an integrity violation | `dht.rs:4097-4223` | Filed — note for future hardening. |
| FM-A23 | `chiral_site_directory` registry blindly trusts whatever JSON is at the registry key, allowing soft DoS via a giant array | `lib.rs:4504-4522` | Filed — cap registry size on read. |
...
- **6 fixed** in this session: FM-A01/02 (Drive share spent-tx ledger), FM-A06 (`dht_put` namespace), FM-A10/A11 (CDN tolerance + f64), FM-A13 (version middleware effective vs bundled), FM-A19 (FileMetadata length-prefix).
- **15 filed** for follow-up — most require coordinated refactors (signed-challenge auth, FileInfo signing, allowlists, registry validators) larger than a single audit-and-fix session can responsibly land.
- **2 already-fixed before this session** — BUG-001 (site-name hijack) and BUG-002 (canonical_signing_payload NUL collision), filed earlier.
- **No false positives.** Every finding above was confirmed by direct code reading at the cited line ranges.
